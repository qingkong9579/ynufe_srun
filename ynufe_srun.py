...
if __name__ == '__main__':
	...
	print("用户名:", info['user_name'] if 'user_name' in info else '')
	print("产品名称:", info['products_name'] if 'products_name' in info else '')
	print("用户MAC:", info['user_mac'] if 'user_mac' in info else '')
	print("在线IP:",info['online_ip'] if 'online_ip' in info else '') 
	print("在线设备数", info['online_device_total'] if 'online_device_total' in info else '')
	print("登陆时间:", datetime.fromtimestamp(info['add_time']).strftime('%Y-%m-%d %H:%M:%S') if 'add_time' in info else '')
	print("在线时长:", seconds_to_hms(info['keepalive_time']-info['add_time']) if 'keepalive_time' in info else '')
	print("已用流量:", convert_bytes_to_human_readable(sum_bytes) if 'sum_bytes' in info else '')
	print("已用时长:", seconds_to_hms(info['sum_seconds']) if 'sum_seconds' in info else '') 
	print("程序执行完毕")
	input("按Enter键退出...")
